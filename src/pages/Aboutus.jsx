...
import { setIsMobileView } from "../Redux/Features/isMobileViewSlice";  // import the action

const AboutUs = () => {
  let { search } = useLocation();
  const searchParams = new URLSearchParams(search);
  const mobileQuery = searchParams.get("mobile") ?? "false";
  
  const dispatch = useDispatch();
  const isMobileView = useSelector(state => state.isMobileView.isMobileView);

  // Set Redux state when component mounts
useEffect(() => {
    dispatch(setIsMobileView(mobileQuery === "true"));
  }, [mobileQuery, dispatch]);
    ...
    <>
      ...
      <Container maxW={"container.xl"} mb={4} px={0} centerContent>
        ...
        <Flex flexDirection={"column"} gap={3} px={{ md: "10%", base: 10 }} py={10}>
          <Box fontWeight={"600"} color="text.300" fontSize={"25px"} alignContent={"flex-start"}>
            <i>Co Fee Co has Dakshin Bharatiya origins:- </i>
          </Box>

          <Box color={"text.300"} textAlign={"justify"}>
            Our blends are made with some of the finest, most authentic coffee beans sourced from the Southern regions of Bharat.Ever since Sufi
            Saint Baba Budan brought the first 7 coffee beans from Arabia to Bharat three and a half centuries ago, Southern Bharat is a
            favourite among coffee lovers globally. Coffee sourced from this region has a unique flavor which is less acidic and sweeter than the
            African and South American varieties. The aroma reminds you of Dakshin Bharat, its verdant hills, and rich cultural heritage.
            <br />
            <br />
            <b style={{ textDecoration: "underline", fontSize: "18px" }}>Packaging Details:-</b>
            <br />
            <br />
            Co Fee Co is committed to bringing you the very best and most refreshing coffee and herbs with minimal impact to the environment.
            This carton is made from 100% recycled paperboard. Our infusion bas are made from unbleached, biodegradable fiber. Please recycle.
            <br />
            <br />
            <b style={{ textDecoration: "underline", fontSize: "18px" }}>Reduce Plastic Usage:-</b>
            <br />
            <br />
            Co Fee Co fully supports the Government of Bharat’s efforts to reduce the use of plastic. Although recycling helps to reduce the
            amount of plastic which ends up clogging our ecosystem, we realize that very few types of plastics can actually be recycled by most
            municipalities. In addition, recycling consumes a lot of energy and water. In this situation, it's best to reduce or completely eliminate
            plastic usage.
            <br />
            <br />
        ...
        <hr />
        <Box w="100%" backgroundSize="100%" backgroundPosition="50% 100%" backgroundRepeat={"no-repeat"}>
          <Heading color="brand.500" size="lg" mx="auto" align={"center"} my={"5"} pb={"10px"}>
            AVAILABLE AT
          </Heading>
        </Box>
